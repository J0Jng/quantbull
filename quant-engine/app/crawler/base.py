"""
Base crawler class for data collection in quant engine.
"""
...
class BaseCrawler(ABC):
    """
    Abstract base class for all data crawlers in quant engine.
    
    Used for collecting market data, factor data, or other
    quantitative data needed for backtesting and strategy execution.
    """
        ...
        try:
            ...
            for record in raw_data:
                ...
                if not self.validate(record):
                    self.logger.warning(f"Invalid record skipped: {record.get('code', 'unknown')}")
                    continue
